...
def find_first_unsolved(p_grid):
    ...
    if unsolved_idx != -1:
        lowest_possible_values = [
            idx for idx, value in enumerate(p_grid[unsolved_idx]) if value == 1
        ]
    else:
        lowest_possible_values = []
...
def simple_possibility_pruner(possibility_grid):
    for sudoku_index, possibilities in enumerate(possibility_grid):
        ...
        new_possibilities = [
            int(current and not new)
            for current, new in zip(possibilities, exclusion_mask)
        ]
...
def get_block(x, y, p_grid):
    ...
    indeces = [
        ...
        for x_shift, y_shift in product(range(3), range(0, 27, 9))
    ]
    indeces.sort()
