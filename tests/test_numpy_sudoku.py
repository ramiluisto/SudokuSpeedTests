...
import src.numpy_sudoku as sudoku_solver
...
def p_grid_elt_translator(p_array):
    if p_array.sum() == 1:
...
def test_p_grid_elt_translator():
    ...
    assert p_grid_elt_translator(np.zeros(9)) == 0
    non_unique_test = np.zeros(9)
    non_unique_test[1] = 1
    non_unique_test[5] = 1
    assert p_grid_elt_translator(non_unique_test) == 0
...
def test_create_copy_of_sudoku(simple_sudoku_object):
    ...
    for cell_idx in range(81):
        copied_sudoku.sudoku[cell_idx // 9][
            cell_idx % 9
        ] == simple_sudoku_object.sudoku[cell_idx // 9][cell_idx % 9]
